...
# Average F1 score of all annotations
def do_eval_avg(gt_dict, pred_dict, threshold=0.05):
    # recall precision f1 for threshold 0.05(5%)
    tp_all = 0
    num_pos_all = 0
    num_det_all = 0
    ...
    for vid_id in list(gt_dict.keys()):
        ...
        tp_all += np.mean(tp_tmplist)
        num_pos_all += np.mean(num_pos_tmplist)
...
# Only choose the annotation with the highest F1 consistency score
def do_eval_high(gt_dict, pred_dict, threshold=0.05):
    # recall precision f1 for threshold 0.05(5%)
    tp_all = 0
    num_pos_all = 0
    num_det_all = 0
    ...
    for vid_id in list(gt_dict.keys()):
        ...
        highest = np.argmax(gt_dict[vid_id]['f1_consis'])
        
        bdy_timestamps_list_gt = bdy_timestamps_list_gt_allraters[highest]
        num_pos = len(bdy_timestamps_list_gt)
        tp = 0
        offset_arr = np.zeros((len(bdy_timestamps_list_gt), len(bdy_timestamps_det)))
        for ann1_idx in range(len(bdy_timestamps_list_gt)):
        ...
        num_pos_tmplist[highest] = num_pos
        fn = num_pos - tp
        fp = num_det - tp
        if num_pos == 0:
            rec = 1
        ...
        if (rec + prec) == 0:
            ...
            f1 = 2 * rec * prec / (rec + prec)
        tp_tmplist[highest] = tp
        f1_tmplist[highest] = f1

        tp_all += tp_tmplist[highest]
        num_pos_all += num_pos_tmplist[highest]
...
def eval_f1(my_pred, gt_path, threshold=0.5, return_pred_dict=False, rel_dis_thres=0.05, protocol='max'):
    ...
    for thres in rel_dis_thres:
        if protocol == 'max':
            ...
            f1, rec, prec = do_eval_avg(gt_dict, pred_dict, threshold=thres)
            results[thres] = f1, rec, prec
        elif protocol == 'high':
            f1, rec, prec = do_eval_high(gt_dict, pred_dict, threshold=thres)
            results[thres] = f1, rec, prec
